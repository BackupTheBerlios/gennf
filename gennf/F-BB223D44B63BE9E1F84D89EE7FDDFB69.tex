...
and deleting of files only by using the explicit subcommands delete
and move. 
...
\end{verbatim}}
  ...
  While testing the developed frontend we noticed a strange behaviour
  of \GENNF{} when using \textsc{emacs} for editing files.
  \textsc{emacs} renames the original file by appending a \~ and then
  copies the file with its original file name. All changes are now
  made to the copy of the controlled file. \textsc{emacs} behaves
  this way because it does not recopgnize that the file is under
  version control. If \textsc{emacs} recognizes a know version tree it will not
  show this behavior.
